...
//        }
    ...
    public void onClick(View v) {
        ...
        if (object != null && object instanceof ViewHolder) {
            ...
            if(holder.tabIndex==3&&TextUtils.isEmpty(AppSharePerference.getUserToken())){
                ((BaseActivity)getContext()).toActivity(LoginActivity.newIntent());
            }else{
                showFragment(holder);
    ...
    public void toFragment(String tage,int cateType){
        for (int i = 0; i < mViewHolderList.size(); i++) {
            if(TextUtils.equals(mViewHolderList.get(i).tag,tage)){
                showFragment(mViewHolderList.get(i),cateType);
            }
        }
    }
    ...
    private void showFragment(ViewHolder holder,int cateType) {
        FragmentTransaction transaction = mFragmentActivity.getSupportFragmentManager().beginTransaction();
        if (isFragmentShown(transaction, holder.tag)) {
            return;
        }
        ...
        bundle.putInt("cateType",cateType);
        Fragment fragment = mFragmentActivity.getSupportFragmentManager().findFragmentByTag(holder.tag);
        if (fragment == null) {
            fragment = getFragmentInstance(holder.tag);
